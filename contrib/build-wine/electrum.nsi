...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-RAY"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-ray-${PRODUCT_VERSION}-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-ray-${PRODUCT_VERSION}.exe" ""
  ...
  WriteRegStr HKCU "Software\Classes\raycoin" "" "URL:raycoin Protocol"
  WriteRegStr HKCU "Software\Classes\raycoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\raycoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\raycoin\shell\open\command" "" "$\"$INSTDIR\electrum-ray-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\raycoin"
